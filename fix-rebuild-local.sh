...
p=Path(sys.argv[1])
s=p.read_text()
# Normalize the existing compact bot by replacing exact one-line functions and the whole dispatcher.
...
if old_menu not in s:
 raise SystemExit('menu marker not found in telegram_bot.py')
s=s.replace(old_menu,new_menu,1)
# Add helpers immediately before deliver().
...
# Replace the whole existing process() body up to loop().
a=s.find('def process(u):')
b=s.find('def loop():',a)
...
s=s[:a]+proc+s[b:]
# Make menu actually admin-aware by letting send() choose admin menu only through /admin; main menu remains user-facing.
p.write_text(s)
PY
