...
def layout(title,body,p):
 it=cfg();return render_template_string(CSS+'''<div class=shell><aside class=side><div class=brand><div class=logo>AW</div><div><h2>AWG Panel <span>7.1</span></h2><small>Mobile | AWG 3.1 Strong</small></div></div><nav class=menu>{{nav|safe}}</nav><div class=sidebox><span class=dot></span>Статус сервера<div style="font-size:18px;margin-top:9px">AWG: <b class="{{'ok' if on else 'bad'}}">{{'online' if on else 'offline'}}</b></div><div class=muted style="margin-top:8px">Интерфейс: awg0<br>Порт: {{port}} UDP<br>Клиентов: {{count}}</div></div></aside><main class=main><header class=top><div>{{title}}</div><div class=right><span class=pill><span class=dot></span>{{'Система: OK' if on else 'AWG offline'}}</span><a class=btn href=/logout>↪ Выйти</a></div></header><div class=wrap>{{body|safe}}<div class=footer>AWG Panel 7.1 | AmneziaWG 3.1 | FREE YOUR MIND</div></div></main></div>''',nav=nav(p),title=title,body=body,on=online(),port=it.get('ListenPort','-'),count=len(rows()))
@app.before_request
def auth():
...
def settings():
 if request.method=='POST':
  c=db();c.execute("UPDATE settings SET v=? WHERE k='login'",(request.form.get('login','admin'),));c.execute("UPDATE settings SET v=? WHERE k='endpoint'",(request.form.get('endpoint',''),));c.commit();c.close();return redirect('/settings')
