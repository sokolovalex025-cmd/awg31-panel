...
import json,time,socket,subprocess,shutil,hashlib
from pathlib import Path
from flask import jsonify
BASE=Path('/opt/awg31-panel');REPO=Path('/root/awg31-panel');BACKUP=BASE/'backups';BACKUP.mkdir(parents=True,exist_ok=True)
...
def cmd(*args,timeout=8):
 try:
  p=subprocess.run(args,text=True,capture_output=True,timeout=timeout);return p.returncode,(p.stdout+p.stderr).strip()[-4000:]
...
def tcp(host,port,timeout=3):
 t=time.monotonic()
 try:s=socket.create_connection((host,port),timeout=timeout);s.close();return {'ok':True,'ms':round((time.monotonic()-t)*1000,1)}
 except Exception as e:return {'ok':False,'error':str(e),'ms':None}
def iface():
 rc,out=cmd('awg','show','awg0');return {'ok':rc==0,'raw':out[-5000:]}
def status():
 endpoints=[{'name':n,'host':h,'port':p,'tcp':tcp(h,p)} for n,h,p in TARGETS];mtu=[]
 ...
 services={x:service_state(x) for x in WATCH};score=(25 if iface()['ok'] else 0)+(25 if services.get('awgpanel.service')=='active' else 0)+(20 if services.get('nginx.service')=='active' else 0)+(15 if any(x['tcp']['ok'] for x in endpoints) else 0)+(15 if all(x['ok'] for x in mtu) else 0)
...
def doctor():
 ...
 rc,out=cmd('awg','show','awg0');add('AWG command',rc==0,'awg0 readable' if rc==0 else out,'Install/repair AmneziaWG 3.1.')
 s=service_state('awg-quick@awg0.service');add('AWG service',s=='active',s,'Restart awg-quick@awg0.')
 s=service_state('awgpanel.service');add('Panel service',s=='active',s,'Restart awgpanel.')
 s=service_state('nginx.service');add('Nginx',s=='active',s,'Check nginx -t and restart nginx.')
 rc,out=cmd('sysctl','-n','net.ipv4.ip_forward');add('IPv4 forwarding',rc==0 and out.strip()=='1',out,'Run nova-network-fix.sh.')
 rc,out=cmd('ip','-4','route','show','default');parts=out.split();wan=parts[parts.index('dev')+1] if 'dev' in parts else '';add('WAN route',bool(wan),wan or out,'Check default route.')
 if wan:
...
def apply(app):
 ...
 def metrics_history():return jsonify({'timestamp':int(time.time()),'current':status()})
 return True
