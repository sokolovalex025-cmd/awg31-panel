...
check systemctl is-active awg-quick@awg0.service
...
if "$PY" "$BASE/nova_awg31_fix.py" check >/tmp/nova-awg31-check.out 2>&1; then ok 'AWG 3.1 live parameters'; else bad "AWG 3.1 live parameters: $(tr '\n' ' ' </tmp/nova-awg31-check.out | cut -c1-240)"; fi
check systemctl is-enabled awgpanel.service
check systemctl is-enabled nginx.service
check systemctl is-enabled nova-watchdog.timer
