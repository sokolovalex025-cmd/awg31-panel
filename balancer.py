...
BASE=Path('/opt/awg31-panel'); DB=BASE/'panel.db'; CONF_DIR=BASE/'balancer'; CONF_DIR.mkdir(parents=True,exist_ok=True)
TOKEN_FILE=Path('/etc/awg31-panel/balancer.env')
...
def _init():
    ...
    CREATE TABLE IF NOT EXISTS balancer_nodes(
      ...
      api_port INTEGER NOT NULL DEFAULT 9090,weight INTEGER NOT NULL DEFAULT 100,enabled INTEGER NOT NULL DEFAULT 1,
      status TEXT NOT NULL DEFAULT 'unknown',latency_ms INTEGER,clients INTEGER NOT NULL DEFAULT 0,
      rx INTEGER NOT NULL DEFAULT 0,tx INTEGER NOT NULL DEFAULT 0,last_check INTEGER NOT NULL DEFAULT 0,
      created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')));
    CREATE TABLE IF NOT EXISTS balancer_assignments(
      client_id INTEGER PRIMARY KEY,node_id INTEGER NOT NULL,assigned_at INTEGER NOT NULL DEFAULT (strftime('%s','now')));
    '''); c.commit(); c.close()
...
def _request(row,path,method='GET',payload=None):
    ...
    req=urllib.request.Request(_url(row,path),data=data,headers=headers,method=method)
    try:
        with urllib.request.urlopen(req,timeout=3.5) as r: return json.loads(r.read().decode() or '{}')
    except Exception: return None

def _health(row):
    started=time.monotonic(); data=_request(row,'/health'); status='online' if data and data.get('ok') else 'down'
    latency=int((time.monotonic()-started)*1000) if status=='online' else None
    clients=int(data.get('clients',0)) if data else 0; rx=int(data.get('rx',0)) if data else 0; tx=int(data.get('tx',0)) if data else 0
...
def _nodes():
    c=_db(); r=c.execute('SELECT * FROM balancer_nodes ORDER BY id').fetchall(); c.close(); return r

def node_by_id(node_id):
    c=_db(); r=c.execute('SELECT * FROM balancer_nodes WHERE id=?',(node_id,)).fetchone(); c.close(); return r
...
def node_add_client(row,pub,psk,addr): return bool(_request(row,'/client/add','POST',{'public_key':pub,'psk':psk,'address':addr}) and _request(row,'/client/add','POST',{'public_key':pub,'psk':psk,'address':addr}).get('ok'))
...
def choose_node():
    ...
    for n in _nodes():
        ...
        weight=max(1,int(n['weight'] or 1)); score=(int(n['clients'] or 0)+1)/weight+(int(n['latency_ms'] or 9999)/100000.0)
