...
for f in app.py nova11.py panel_bootstrap.py nova12_theme.py nova13_theme.py nova14_theme.py nova15_theme.py nova16_server_card_fix.py nova_awg31_fix.py antiblock.py nova_shield.py nova_resilience.py domain_manager.py telegram_ui.py telegram_bot.py telegram_runner.py telegram_delete.py keenetic.py balancer.py balancer_provision.py nova_mobile_diagnostics.py background.svg nova-network-fix.sh nova-max-backup.sh nova-migrate.sh nova-verify.sh; do
  [ -f "$REPO_DIR/$f" ] && install -m 644 "$REPO_DIR/$f" "$BASE/$f"
done
chmod 755 "$BASE/nova_awg31_fix.py" "$BASE/nova-network-fix.sh" "$BASE/nova-max-backup.sh" "$BASE/nova-migrate.sh" "$BASE/nova-verify.sh" "$BASE/panel_bootstrap.py"

"$REPO_DIR/venv/bin/python" -m py_compile "$BASE"/*.py

# Apply the actual AWG 3.1 changes. The helper creates a backup and restores
# the previous config automatically if the running interface rejects them.
...
echo 'The GitHub version has been applied to the VPS.'
