...
PAY='https://raw.githubusercontent.com/sokolovalex025-cmd/awg31-panel/main/telegram_payments.py'
[ -f "$FILE" ] || { echo "Missing $FILE"; exit 1; }
mkdir -p "$BASE/backups"
cp -a "$FILE" "$BASE/backups/telegram_bot-before-full-v2-$(date +%Y%m%d-%H%M%S).py"
TMP=$(mktemp)
PAYTMP=$(mktemp)
trap 'rm -f "$TMP" "$PAYTMP"' EXIT
curl -fsSL --retry 3 "$REPO" -o "$TMP"
curl -fsSL --retry 3 "$PAY" -o "$PAYTMP"
python3 - "$FILE" "$TMP" "$PAYTMP" <<'PY'
from pathlib import Path
import sys
live=Path(sys.argv[1]); source=Path(sys.argv[2]); pay=Path(sys.argv[3])
s=source.read_text()
