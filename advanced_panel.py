...
def traffic():
 ...
 return out
def backup():
 b=BASE/'backups';b.mkdir(exist_ok=True);p=b/f'awgpanel-{time.strftime("%Y%m%d-%H%M%S")}.tar.gz'
...
def register(app):
 ...
 try:
  ...
  if not getattr(main,'_advanced_nav_patched',False):
   def nav(p):return old(p)+f'<a class="{"active" if p=="/dashboard-plus" else ""}" href="/dashboard-plus">◈&nbsp;&nbsp;Dashboard Pro</a>'
