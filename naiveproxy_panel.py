...
def valid_password(v):return bool(re.fullmatch(r'[A-Za-z0-9._-]{8,64}',v))
def status():return run('systemctl','is-active',SERVICE).stdout.strip()=='active'
def status_text():return run('systemctl','is-active',SERVICE).stdout.strip() or 'unknown'
def service_error():
    r=run('systemctl','status',SERVICE,'--no-pager','-n','12',timeout=8);return (r.stdout or r.stderr or 'Нет данных')[-5000:]
def meta():
...
def install(domain,email,user,password,port):
    ...
    if not valid_password(password):return False,'Пароль: 8–64 символа A-Z/a-z/0-9/._-.'
    try:port=int(port)
    ...
    if r.returncode:return False,'apt-get update завершился ошибкой: '+(r.stderr or r.stdout)[-1200:]
    r=run('apt-get','install','-y','golang-go','git','curl','ca-certificates','libcap2-bin',timeout=600)
    ...
    web=Path('/var/www/naiveproxy');web.mkdir(parents=True,exist_ok=True);run('chown','-R','caddy:caddy',str(web))
    (web/'index.html').write_text('<!doctype html><meta charset="utf-8"><title>AWG Panel</title><main>NaïveProxy is running.</main>');run('chown','caddy:caddy',str(web/'index.html'))
    CFG.write_text(f'''{{\n    order forward_proxy before file_server\n    email {email}\n}}\n\n:{port}, {domain} {{\n    encode\n    forward_proxy {{\n        basic_auth {user} {password}\n        hide_ip\n        hide_via\n        probe_resistance\n    }}\n    file_server {{\n        root /var/www/naiveproxy\n    }}\n}}\n''')
    try:os.chown(CFG,0,__import__('grp').getgrnam('caddy').gr_gid)
    ...
    run('systemctl','daemon-reload');run('systemctl','enable',SERVICE)
    check=run(str(BIN),'validate','--config',str(CFG),timeout=15)
    if check.returncode:return False,'Конфигурация NaïveProxy не прошла проверку: '+(check.stderr or check.stdout)[-1800:]
    r=run('systemctl','restart',SERVICE)
    if r.returncode or not status():return False,'NaïveProxy не запустился. '+service_error()
...
def register(app):
    ...
    if hasattr(mod,'nav') and not getattr(mod.nav,'_naive_wrapped',False):
        original=mod.nav
        def nav2(p):
            s=original(p);link=f'<a class="{"active" if p=="/naiveproxy" else ""}" href="/naiveproxy">🚀&nbsp;&nbsp;NaïveProxy</a>';return s+link
