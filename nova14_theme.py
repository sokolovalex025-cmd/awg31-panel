#!/usr/bin/env python3
"""NOVA 14 visual layer: Telegram, Domains, AntiBlock and robust sidebar."""
CSS = r'''<style>
...
.antiblock-nav{background:linear-gradient(100deg,rgba(255,123,95,.16),rgba(255,196,94,.07))!important;border-color:rgba(255,161,95,.28)!important;color:#ffe1c5!important}.antiblock-nav.active{box-shadow:inset 3px 0 #ff9d66,0 8px 24px rgba(255,130,80,.10)!important}
...
def apply(nova):
    ...
    def nav(path):
        ...
        items=[
          ...
          '<a class="antiblock-nav %s" href="/antiblock"><i>🛡</i>AntiBlock</a>' % ('active' if path.startswith('/antiblock') else '')
        ]
        for item in items:
