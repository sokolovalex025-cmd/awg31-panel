 ...
 while True:
  ...
  for u in r.get('result',[]):
   ...
   if 'callback_query' in u:
    ...
    if not allowed(uid):send(chat,'⛔ Доступ запрещён.',False);continue
    if cmd in ('/start','/help'):send(chat,'🤖 <b>AWG Panel Bot</b>\n\nУправление сервером и мониторинг AWG/NaïveProxy.')
    elif cmd=='/status':send(chat,text_status())
    elif cmd=='/restart':send(chat,'🔄 AWG перезапущен.' if run('systemctl','restart','awg-quick@awg0').returncode==0 else '❌ Ошибка перезапуска.')
    else:send(chat,'Используйте /status или кнопки меню.')
def register(app):
 mod=__import__('app')
 ...
 def telegram_page():
  if request.method=='POST':
   ...
   if action=='save':save_env(request.form.get('token','').strip(),request.form.get('ids','').strip());run('systemctl','daemon-reload');run('systemctl','restart',SERVICE)
