...
BASE=Path('/opt/awg31-panel');ENV=Path('/etc/awg31-panel/telegram.env');SERVICE='awgpanel-telegram.service';DB=BASE/'panel.db';CONF=Path('/etc/amnezia/amneziawg/awg0.conf')
...
def tg(method,data=None,timeout=40):
 ...
 if not token:return {}
 try:
  req=urllib.request.Request(f'https://api.telegram.org/bot{token}/{method}',data=urllib.parse.urlencode(data or {}).encode(),headers={'Content-Type':'application/x-www-form-urlencoded'})
...
def allowed(uid):
 a=ids('TELEGRAM_ALLOWED_IDS');return not a or uid in a or uid in ids('TELEGRAM_ADMIN_IDS')
def admin(uid):return uid in ids('TELEGRAM_ADMIN_IDS')
def db():c=sqlite3.connect(DB);c.row_factory=sqlite3.Row;return c
def init_db():
 c=db();c.execute('''CREATE TABLE IF NOT EXISTS telegram_subscriptions(telegram_id INTEGER PRIMARY KEY,username TEXT,client_id INTEGER NOT NULL,expires INTEGER NOT NULL,active INTEGER NOT NULL DEFAULT 1,created INTEGER NOT NULL,peer_block TEXT DEFAULT '')''')
...
def panel_request(path,method='GET',data=None):
 import app as panel
 client=panel.app.test_client();r=client.post('/login',data={'login':panel.setting('login','admin'),'password':panel.setting('password','change-me')})
 if r.status_code not in (301,302):raise RuntimeError('panel login failed')
 return client.post(path,data=data or {},follow_redirects=False) if method=='POST' else client.get(path)
def create_client(name):
 panel_request('/clients/create','POST',{'name':name});c=db();r=c.execute('SELECT * FROM clients WHERE name=? ORDER BY id DESC LIMIT 1',(name,)).fetchone();c.close()
...
def peer_block(pub):
 ...
 for p in CONF.read_text(errors='replace').split('[Peer]')[1:]:
  block='[Peer]'+p
  if re.search(r'^\s*PublicKey\s*=\s*'+re.escape(pub)+r'\s*$',block,re.M):return block.strip()+'\n'
 return ''
def sync_conf():
 r=run('bash','-lc','tmp=$(mktemp); trap "rm -f $tmp" EXIT; awg-quick strip awg0 > "$tmp" && awg syncconf awg0 < "$tmp"')
 return r.returncode==0
def set_peer_enabled(pub,enabled,stored=''):
 if not CONF.exists():return False
 text=CONF.read_text(errors='replace');parts=text.split('[Peer]');found=False;out=[parts[0].rstrip()]
 for p in parts[1:]:
  block='[Peer]'+p
 ...
 if not enabled and not found:return True
...
def save_sub(uid,username,cid,expires,block):
 c=db();now=int(time.time());c.execute('''INSERT INTO telegram_subscriptions(telegram_id,username,client_id,expires,active,created,peer_block) VALUES(?,?,?,?,1,?,?) ON CONFLICT(telegram_id) DO UPDATE SET username=excluded.username,client_id=excluded.client_id,expires=excluded.expires,active=1,peer_block=excluded.peer_block''',(uid,username or '',cid,expires,now,block));c.commit();c.close()
def issue(uid,username,days):
 now=int(time.time());s=subscription(uid)
 if s and client_row(s['client_id']):
  c=client_row(s['client_id']);block=s['peer_block'] or peer_block(c['public_key']);set_peer_enabled(c['public_key'],True,block);expires=max(now,s['expires'])+days*86400;save_sub(uid,username,c['id'],expires,block);return c['id'],expires,False
 r=create_client('TG-'+str(uid));block=peer_block(r['public_key']);expires=now+days*86400;save_sub(uid,username,r['id'],expires,block);return r['id'],expires,True
def menu():return json.dumps({'inline_keyboard':[[{'text':'🔐 Получить VPN','callback_data':'get'}],[{'text':'📱 Мой VPN','callback_data':'my'},{'text':'♻️ Продлить','callback_data':'renew'}],[{'text':'ℹ️ Помощь','callback_data':'help'}]]},ensure_ascii=False)
def send(chat,text,markup=True):tg('sendMessage',{'chat_id':chat,'text':text,'parse_mode':'HTML','reply_markup':menu() if markup else ''})
def multipart(method,data,files):
 token=env().get('TELEGRAM_BOT_TOKEN','');boundary='----NOVA'+str(int(time.time()*1000));body=bytearray()
 for k,v in data.items():body+=(f'--{boundary}\r\nContent-Disposition: form-data; name="{k}"\r\n\r\n{v}\r\n').encode()
 for k,(fn,content,mime) in files.items():body+=(f'--{boundary}\r\nContent-Disposition: form-data; name="{k}"; filename="{fn}"\r\nContent-Type: {mime}\r\n\r\n').encode()+content+b'\r\n'
 body+=(f'--{boundary}--\r\n').encode()
 try:
...
def deliver(chat,uid,username):
 try:
  cid,expires,_=issue(uid,username,int(env().get('VPN_DEFAULT_DAYS','30')));conf=conf_bytes(cid);qr=qr_bytes(cid)
  tg('sendMessage',{'chat_id':chat,'text':f'✅ <b>VPN готов</b>\nСрок до: <b>{fmt(expires)}</b>\nAWG 3.1 · Strong Mobile','parse_mode':'HTML'})
  multipart('sendDocument',{'chat_id':chat,'caption':'📄 Конфигурация AmneziaWG'},{'document':('NOVA-'+str(cid)+'.conf',conf,'text/plain')})
  multipart('sendPhoto',{'chat_id':chat,'caption':'📲 QR-код для подключения'},{'photo':('NOVA-'+str(cid)+'.png',qr,'image/png')})
...
def show_my(chat,uid):
 ...
 c=client_row(s['client_id']);active=bool(s['active']) and s['expires']>int(time.time());send(chat,f'📱 <b>{c["name"] if c else "NOVA VPN"}</b>\nIP: <code>{c["address"] if c else "-"}</code>\nСтатус: <b>{"🟢 Активен" if active else "🔴 Истёк"}</b>\nДо: <b>{fmt(s["expires"])}</b>',True)
def renew(chat,uid,username):
 s=subscription(uid)
 if not s:send(chat,'Сначала нажмите «🔐 Получить VPN».',True);return
 try:
  _,expires,_=issue(uid,username,int(env().get('VPN_DEFAULT_DAYS','30')));send(chat,f'♻️ <b>VPN продлён</b>\nЕщё {env().get("VPN_DEFAULT_DAYS","30")} дней.\nДо: <b>{fmt(expires)}</b>',True)
...
def process(u):
 ...
 if 'message' in u:
  ...
  if cmd in ('/start','/help'):send(chat,'🚀 <b>NOVA Network Control Center</b>\n\nУправление AmneziaWG 3.1 прямо из Telegram.',True)
   ...
   c=db();n=c.execute('SELECT COUNT(*) FROM telegram_subscriptions').fetchone()[0];c.close();send(chat,f'🛠 Подписчиков: <b>{n}</b>',True)
  else:send(chat,'Используйте кнопки меню.',True)
def loop():
 init_db();offset=0;last=0
 while True:
  if time.time()-last>60:expire_once();last=time.time()
 ...
 def telegram_page():
  if request.method=='POST':
   ...
   if action=='save':
    ...
    try:r=json.loads(urllib.request.urlopen(urllib.request.Request('https://api.telegram.org/bot'+token+'/getMe'),timeout=8).read().decode())
    except Exception:r={}
