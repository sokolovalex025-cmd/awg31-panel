...
# Sidebar UX: scrollbar is on the RIGHT of the menu. The AWG status card is
# kept outside the scrollable nav so it never covers the last menu entries.
core.CSS = core.CSS.replace(
    '.sidebar{position:fixed;',
    '.sidebar{position:fixed;overflow:hidden;display:flex;flex-direction:column;'
)
core.CSS = core.CSS.replace(
    '.nav{display:grid;',
    '.nav{display:grid;flex:1;min-height:0;overflow-y:auto;padding-right:4px;padding-bottom:10px;scrollbar-width:thin;scrollbar-color:#33445c transparent;'
)
core.CSS = core.CSS.replace(
    '.server{position:absolute;left:14px;right:14px;bottom:17px;',
    '.server{position:relative;left:auto;right:auto;bottom:auto;flex:0 0 auto;margin:8px 0 0;'
)
# Remove the previous left-scrollbar RTL hack if this file is upgraded in-place.
core.CSS = core.CSS.replace('direction:rtl;padding-bottom:120px;scrollbar-width:thin;scrollbar-color:#33445c transparent;', '')
core.CSS = core.CSS.replace('.sidebar>*{direction:ltr}', '')
core.CSS += '''<style>
.nav::-webkit-scrollbar{width:7px}
.nav::-webkit-scrollbar-track{background:transparent}
.nav::-webkit-scrollbar-thumb{background:#33445c;border-radius:10px;border:2px solid transparent;background-clip:padding-box}
.nav::-webkit-scrollbar-thumb:hover{background:#4f8cff;background-clip:padding-box}
@media(max-width:760px){.sidebar{display:block;overflow-y:auto}.nav{display:grid;overflow:visible;max-height:none;padding-right:0}.server{margin-top:8px}}
...
def nova_diagnostics_api():
    sm=_strong_mobile_status(); awg_ok=core.online(); c=core.cfg(); port=c.get('ListenPort','1234')
    ss=core.cmd('ss','-lun'); udp_ok=ss.returncode==0 and any((':'+str(port)+' ') in x or ('0.0.0.0:'+str(port)) in x for x in ss.stdout.splitlines())
    ps=core.peers(); now=int(time.time()); handshakes=sum(1 for x in ps.values() if x.get('handshake') and now-x['handshake']<=180)
