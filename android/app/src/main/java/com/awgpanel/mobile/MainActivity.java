...
public class MainActivity extends Activity {
    private static final String PREFS = "awg_panel";
    ...
    private void buildUi() {
        ...
        top.setPadding(dp(16), dp(6), dp(10), dp(4));
        top.setBackgroundColor(Color.rgb(3, 13, 28));

        LinearLayout titleBox = new LinearLayout(this);
        titleBox.setOrientation(LinearLayout.VERTICAL);
        TextView title = text("AWG Panel", Color.WHITE, 18, true);
        addressText = text(panelUrl(), Color.rgb(145, 165, 185), 11, false);
        addressText.setSingleLine(true);
        titleBox.addView(title);
        titleBox.addView(addressText);
        top.addView(titleBox, new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f));

        Button refresh = toolButton("↻");
        Button settings = toolButton("⚙");
        top.addView(refresh);
        ...
        ws.setTextZoom(100);
        ws.setUserAgentString("Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Mobile Safari/537.36 AWGPanelAndroid/3.0");
        CookieManager cm = CookieManager.getInstance();
        cm.setAcceptCookie(true);
        cm.setAcceptThirdPartyCookies(webView, true);
        webView.setBackgroundColor(Color.rgb(3, 13, 28));
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            @Override public void onPageFinished(WebView v, String url) {
                ...
                progress.postDelayed(() -> progress.setVisibility(View.GONE), 180);
                showWebView();
            }
            @Override public void onReceivedError(WebView v, WebResourceRequest r, WebResourceError e) {
                if (r.isForMainFrame()) showError("Не удалось открыть панель", "Проверьте интернет и адрес VPS.\n\n" + e.getDescription());
            }
            @Override public void onReceivedSslError(WebView v, SslErrorHandler h, SslError e) {
                h.cancel();
                showError("Ошибка HTTPS", "Сертификат панели не прошёл проверку. Проверьте адрес и сертификат.");
            }
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public void onReceivedTitle(WebView view, String t) {
                if (t != null && !t.trim().isEmpty()) addressText.setText(t);
            }
        });
        webView.setDownloadListener((url, userAgent, contentDisposition, mimeType, contentLength) -> {
            try { startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url))); }
            catch (Exception ignored) { showError("Не удалось открыть файл", "На телефоне нет приложения для этого типа файла."); }
        });
        ...
        errorTitle = text("", Color.WHITE, 22, true);
        errorTitle.setGravity(Gravity.CENTER);
        errorText = text("", Color.LTGRAY, 15, false);
        errorText.setGravity(Gravity.CENTER);
        errorText.setPadding(0, dp(14), 0, dp(20));
        Button retry = actionButton("Повторить");
        Button change = actionButton("Изменить адрес");
        errorView.addView(errorTitle);
        errorView.addView(errorText);
        errorView.addView(retry);
        errorView.addView(change);
        ...
        bottom.setPadding(dp(6), dp(4), dp(6), dp(5));
        bottom.setBackgroundColor(Color.rgb(3, 13, 28));
        Button back = toolButton("‹"), forward = toolButton("›"), home = toolButton("⌂"), reload = toolButton("↻");
        bottom.addView(back, new LinearLayout.LayoutParams(0, dp(50), 1f));
        bottom.addView(forward, new LinearLayout.LayoutParams(0, dp(50), 1f));
        bottom.addView(home, new LinearLayout.LayoutParams(0, dp(50), 1f));
        bottom.addView(reload, new LinearLayout.LayoutParams(0, dp(50), 1f));
        ...
        home.setOnClickListener(v -> loadPanel());
        reload.setOnClickListener(v -> webView.reload());
        root.addView(bottom, new LinearLayout.LayoutParams(-1, dp(58)));
    ...
    private TextView text(String value, int color, float size, boolean bold) {
        ...
        t.setText(value); t.setTextColor(color); t.setTextSize(size);
    ...
    private Button toolButton(String value) {
        ...
        b.setText(value); b.setTextColor(Color.WHITE); b.setTextSize(20);
    ...
    private Button actionButton(String value) {
        Button b = new Button(this);
        b.setText(value); b.setAllCaps(false); b.setMinHeight(dp(48));
        return b;
    }
    ...
    private void showSettings() {
        ...
        input.setSingleLine(true); input.setText(panelUrl()); input.setSelectAllOnFocus(true);
        input.setHint("http://IP:8080 или https://домен");
        new AlertDialog.Builder(this)
                .setTitle("Адрес AWG Panel")
                .setMessage("Адрес web-панели VPS")
                .setView(input)
                .setNegativeButton("Отмена", null)
                .setNeutralButton("Сбросить", (d, w) -> { prefs.edit().remove(KEY_URL).apply(); loadPanel(); })
