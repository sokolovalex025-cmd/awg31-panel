...
import subprocess, ipaddress, os, re, html, secrets
import balancer
...
def _token():
    ...
    if TOKEN_FILE.exists():
        for line in TOKEN_FILE.read_text(errors='ignore').splitlines():
            if line.startswith('BALANCER_TOKEN=') and line.split('=',1)[1].strip(): return line.split('=',1)[1].strip()
    token=secrets.token_urlsafe(32);TOKEN_FILE.write_text('BALANCER_TOKEN='+token+'\n');os.chmod(TOKEN_FILE,0o600);return token

def _provision(user,host,port,key,token):
    target=f'{user}@{host}';remote=f"curl -fsSL {RAW_BASE}/provision-balancer-node.sh | bash -s -- {token}"
    ...
    try:
        p=subprocess.run(args,text=True,capture_output=True,timeout=60);log=(p.stdout or '')[-3000:]
        if p.stderr:log+='\n'+p.stderr[-3000:]
