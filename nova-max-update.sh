...
# Always take a lightweight pre-update snapshot before replacing panel runtime files.
if [ -x "$REPO_DIR/nova-max-backup.sh" ]; then "$REPO_DIR/nova-max-backup.sh" || true; fi

git -C "$REPO_DIR" fetch origin main
git -C "$REPO_DIR" reset --hard origin/main
mkdir -p "$BASE" "$BASE/backups"
for f in nova_shield.py nova_resilience.py panel_bootstrap.py nova14_theme.py antiblock.py domain_manager.py nova-max-backup.sh; do
  [ -f "$REPO_DIR/$f" ] && install -m 644 "$REPO_DIR/$f" "$BASE/$f"
done
chmod 755 "$BASE/panel_bootstrap.py" "$BASE/antiblock.py" "$BASE/nova_shield.py" "$BASE/nova_resilience.py" "$BASE/nova-max-backup.sh"
